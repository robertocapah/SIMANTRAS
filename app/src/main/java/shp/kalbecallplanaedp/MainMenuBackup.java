...
public class MainMenuBackup extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, ResultCallback<LocationSettingsResult> {
    ...
    private final int TAMPILGAMBAR = 111;
    private final int QUERY = 222;

    private final int CAMERA_REQUEST_PROFILE = 120;
    private final String IMAGE_DIRECTORY_NAME = "Image Personal";
    final int SELECT_FILE_PROFILE = 6;
    private Bitmap photoProfile, mybitmapImageProfile;
    private byte[] phtProfile;
...
//                    finish();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isMyServiceRunning(MyServiceNative.class)){
            startService(new Intent(MainMenuBackup.this, MyServiceNative.class));
        }
...
//                DownloadManager.ACTION_DOWNLOAD_COMPLETE));
        ...
        pDialog = new ProgressDialog(MainMenuBackup.this, ProgressDialog.THEME_DEVICE_DEFAULT_LIGHT);
...
//        addProductAndOrder();
        ...
        MainMenuRunnable runnable = new MainMenuRunnable(this,TAMPILGAMBAR);
        ...
        MainMenuRunnable runnable2 = new MainMenuRunnable(this,QUERY);
            ...
            public void onDrawerClosed(View drawerView) {
                ...
                if(checkNavItem!= null){
                    switch (checkNavItem.getItemId()) {
                        case R.id.mnLogout:
                            android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(MainMenuBackup.this);
...
//                    previewCaptureImage2(bitmap);
    ...
    private void clearData() {
        Intent intent = new Intent(MainMenuBackup.this, SplashActivity.class);
        DatabaseHelper helper = DatabaseManager.getInstance().getHelper();
...
//                                ToastCustom.showToasty(getApplicationContext(),"Success Push Data",1);
    ...
    private void logout() {
        ...
        /*new clsHelperBL().volleyLogin(MainMenuBackup.this, strLinkAPI, mRequestBody, "Logout....", new VolleyResponseListener() {
            @Override
            public void onError(String message) {
...
//                            loginRepo.createOrUpdate(data);
                            ...
                            new ToastCustom().showToasty(MainMenuBackup.this,txtMessage,4);
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
        ...
        switch (status.getStatusCode()) {
            ...
            case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                ...
                try {
                    ...
                    status.startResolutionForResult(MainMenuBackup.this, REQUEST_CHECK_SETTINGS);
    ...
    class MainMenuRunnable implements Runnable{
        Activity a;
        int intCode;
        MainMenuRunnable(Activity a, int intCode){
            this.a = a;
            this.intCode = intCode;
        }
        @Override
        public void run() {
            if (intCode == TAMPILGAMBAR){
